...
{#if isOpen}
	...
	<div class="dropdown-content">
		...
		<div class="external-links row border-bottom">
			...
			<a href="https://blog.realgolf.games"><span><Fa icon={faBookOpenReader} /></span>{$_("blog")}</a>
			<a href="/faq"><span><Fa icon={faQuestionCircle} /></span>{$_("faq")}</a>
			<a href="https://docs.realgolf.games"><span><Fa icon={faBook} /></span>{$_("documentation")}</a>
		...
		{#if auth}
			<div class="border-bottom logout row">
				<form action="/logout" method="POST" class="logout-form">
					<button class="link"><span><Fa class="Fa" icon={faSignOutAlt} /></span>{$_("sign_out")}</button>
				</form>
			</div>
		{/if}
	</div>
